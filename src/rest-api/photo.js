...
export function getInitPhotosApi() {
  ...
  let fetchFlickr = fetch('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + config.api_key + '&user_id=' + config.user_id + '&per_page=' + page.number_photos + '&page=' + page.current_page + '&text=' + page.search + '&sort=' + page.sort + '&format=json&nojsoncallback=1');
  return fetchFlickr
    .then((response) => response.json())
    .then((photos) => {
      if (photos.code === 2) {
...
export function getPhotoInfoApi(photoId) {
  ...
  return fetchFlickrPhotoInfo
    .then((response) => response.json())
    .then((photoInfo) => {
      let photoInfoResult = photoInfo.photo;
      let photoComments = getPhotoComments(photoId);
      return photoComments.then((comments) => Object.assign({}, photoInfoResult, { comments: comments }));
    });
}
